...
因此，东北相邻的两个槽区不会先膨胀成一个大连通区，再由 PCA 拟合成一条斜线。新算法一旦形成有效等高线种子，不再合并原算法的广域评分连通区，避免重新引入东北斜向桥接线。
...
如果新等高线种子已经识别出东北等槽线、但由于低纬观测稀疏暂未覆盖海南附近，系统只补充原算法的一条低纬 `meridional_valley_track`；不会补充原算法的东北广域评分轴线。
...
如果等高线提取异常或完全没有形成有效种子，运行时自动回退到原有槽线算法，避免业务图层为空。
